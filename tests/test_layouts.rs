...
#[test]
fn folder_markdown_layout_outputs_folder_and_index() {
    let config = Config {
        layout: "folder_markdown".to_string(),
        ..Config::defaults()
    };
...
#[test]
fn one_per_file_layout_emits_one_file_per_blueprint() {
    let config = Config {
        layout: "one_per_file".to_string(),
        ..Config::defaults()
    };
